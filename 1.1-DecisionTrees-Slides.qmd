...
```
:::
This representation can be used to classify some values following the decisions from top to bottom.
...
Common impurity functions for classification trees.

 For node $t$, the node information function $i(t)$ may be defined by

1.  **Entropy function**: $-\sum_{i=1}^K p(k\vert t)\log p(k\vert t)$.

If $p_{j}=0$, use the limit $\lim p_{j} \rightarrow \log p_{j}=0$.

2.  **Misclassification rate**: $\sum_{i=1}^K p(k\vert t)(1-p(k\vert t)),$

3.  **Gini index**: $1-\sum_{i=1}^K p(k\vert t)^2$.
...
- The entropy function (rescaled) is the red curve, 
- The Gini index is the green curve, and
...
## TB 2.2 Stop splitting criteria
...
# ERROR ESTIMATION AND OPTIMIZATION FOR CLASSIFICATION TREES
