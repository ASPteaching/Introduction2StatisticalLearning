---
title: "Introduction to Python - Lab"
subtitle: "Adapted from the 'Introduction to Statistical Learning with Python. Chapter 02 lab'"
jupyter:
  jupytext:
...
**Note** This can be done by running the installer provided with the labs repository  

```
pip install -r requirements.txt 
```

This will install first the required packages and last the "ISLP" package.

You may need to install some other libraries separately:

This lab can be run form the Rmarkdown file or dirctly in the Jupyter notebook by `Ch2-statlearn-lab.ipynb` available from the repository or from the resources page. 
Now run the following code at the command line: 
...
This lab uses some popular packages that you may want to install before you start.

Thes pacakages provide functionalities for computation (`numpy`),  plotting (`matplotlib`), or data frame management (`pandas`) that are native in R.
...
## Rendering an "Rmd" to "pnp"

The current Rmarkdown document will run the python code inside chunks and it will generate the html output.

It is not the best way to run python code (or is it?) but may be a smoother approach for those unfamiliar with the language and its environments.

A more natural alternative is to work with Jupyter notebooks that can also be downloaded from the site or  created from scratch using `JupyterLab`.
...
To run a function called `fun`, we type `fun(input1,input2)`, where the inputs (or *arguments*) `input1` and `input2` tell
`Python` how to run the function.  

A function can have any number of inputs. For example, the
