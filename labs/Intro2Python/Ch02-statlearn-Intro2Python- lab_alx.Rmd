...
**Note** If you run the installer provided with the labs repository  
...
The current Rmarkdown document will generate, when properly rendered a Jupyter notebook.

This notebook is different to the usual rendered document from Rmarkdown

- In general, a rendered Rmarkdown document is a finished document which we do not interact with anymore.

- In this case it creates aJupyter notebook, which, in spirit, **is more like an R notebook** that can be used to work interactively with the data.

Notice that the .Rmd step is really not necessary as is not Rstudio either.

Jupyter notebooks can be created from scratch using `JupyterLab`.
