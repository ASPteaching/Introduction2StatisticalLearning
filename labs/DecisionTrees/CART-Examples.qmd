...
# Predicting car sales (I)

In the following two examples we use the `carseats`datasets to predict the value of `sales`

In this section we tart with the value seen as categorical.

## Data description

```{r assignments}
# This chunk is used to do the assignment of values to R variables
# This allows the code to be more easily adapted to any change of datasets or variables
...
```{r}
kableExtra::kable(table(mydataset[train,"High"]), caption= "Train data: number of observations for each class",
...
```{r fig.DT1bis, fig.cap="Classification tree", fig.height=10, fig.width=12}
require(rpart.plot)
